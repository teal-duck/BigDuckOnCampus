...
public class MuscovyGame extends ApplicationAdapter implements ApplicationListener, InputProcessor {
    ...
    public void render () {
        ...
        if (!(gameState == 101)){
            this.update();  //Waiting for 1 render cycle before initialising anything
        }
        camera.update();
    ...
    public void update(){
        /**
         * Uses gameState to update game as needed for each state.
         * Waits for 1 render cycle before initialising everything (case 102), so the loading screen can be displayed
         */
        switch (gameState){
            case 0:
                break;
	...
	public boolean keyDown(int keycode) {
		switch (gameState){
			...
			case 1:
                ...
                if(keycode == Input.Keys.ESCAPE) gameState = 0;
				break;
			case 2:
